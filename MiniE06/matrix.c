...
void matrix_dgemm_2(int n, double *restrict _C, double *restrict _A, double *restrict _B)
{
    ...
    int i, j, k, i0, j0, k0;
    int blockSize = 32; //Testei com 16 e 64 também, mas 32 foi o que usou menos tempo
    
    for (i0 = 0; i0 < n; i0 += blockSize)
    {
        int imax = (i0 + blockSize > n) ? n : i0 + blockSize;
        for (k0 = 0; k0 < n; k0 += blockSize)
        {
            int kmax = (k0 + blockSize > n) ? n : k0 + blockSize;
