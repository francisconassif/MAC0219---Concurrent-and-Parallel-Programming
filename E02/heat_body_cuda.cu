...
*
* Arquivo: heat_body_cuda.cu
*
* Objetivo: arquivo heat_cuda.cu incrementado
* com o corpo quente.
*
* Compilação: nvcc -o heat_body_cuda heat_body_cuda.cu
* Uso: ./heat_body_cuda <número de pontos> <limite de iterações> <block_size>
*
*****************************************************************************************/
...
e otimizar os acessos à memória, incluindo
a inicialização do corpo quente.
*/
...
unidimensional de initialize e incluir
a manutenção da temperatura do corpo quente.
...
A posição do corpo foi incluída nos parâmetros.
*/
__global__ void jacobi_iteration_kernel(double *h, double *g, int n, int body_start_x, int body_end_x, int body_start_y, int body_end_y)
{
...
{
    FILE *file = fopen("room_body_cuda.txt", "w");  
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
...
int main(int argc, char *argv[])
{
    ...
    {
        // Lança o kernel
        jacobi_iteration_kernel<<<gridSize, blockSize>>>(h, g, n, body_start_x, body_end_x, body_start_y, body_end_y);
